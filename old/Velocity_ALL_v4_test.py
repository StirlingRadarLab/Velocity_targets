...
def refocus_image(img_FFT, max_val, va):
    ...
    phase_corr = np.exp(1j * (2*np.pi/lambda_c) * (R0_ship + R0_ship * quadratic ) )
...
plt.imshow(np.abs(imgVV_sea), cmap = 'gray', 
           vmin = 0, vmax = 2.5*np.nanmean(np.abs(imgVV_sea)))
...
plt.imshow(np.abs(imgVH_sea), cmap = 'gray', 
           vmin = 0, vmax = 2.5*np.nanmean(np.abs(imgVH_sea)))
...
print(f"Estimated observed across-track speed: {v_az_ship:.2f} m/s")
