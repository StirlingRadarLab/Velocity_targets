...
# ----------------------------
def refocus_with_velocity(img, vx, vy, prf=PRF_DEFAULT, fc=FC,
                          platform_v=7090.0, platform_alt=693000.0,
                          poly_order=None, undo_fft_azimuth=True):
    """
    Doppler refocusing of a 2D complex Sentinel-1 imagette using known target velocity (vx, vy).
    ...
    vx, vy : float
        Target velocity components (m/s) in across-track (x) and along-track (y) directions.
    prf : float
        Azimuth PRF (Hz). Default Sentinel-1 1716 Hz.
    fc : float
        Carrier frequency (Hz). Default Sentinel-1 C-band 5.405 GHz.
    platform_v : float
        Satellite platform velocity (m/s). Default Sentinel-1 ~7090 m/s.
    platform_alt : float
        Satellite altitude (m). Default Sentinel-1 ~693 km.
    undo_fft_azimuth : bool
        True if imagette was FFT-azimuth compressed; False if raw SLC slow-time lines.

    Returns:
    --------
    img_refocused : 2D complex array (same shape as img)
        Doppler-refocused imagette using known velocity.
    info : dict
        Dictionary with computed phase correction, peak range bin, etc.
    """
    import numpy as np
    from numpy.fft import fft, ifft, fftshift, ifftshift
    ...
    # Step 1: optionally undo FFT-based azimuth compression
    if undo_fft_azimuth:
        S_est = fftshift(fft(ifftshift(img, axes=0), axis=0), axes=0)
    else:
        S_est = img.copy()
    ...
    # Step 3: Compute model-based slant-range R(t) per azimuth line
    # Assume platform along x-axis, y is along-track, z is altitude
    xp = platform_v * t_az
    yp = np.zeros_like(t_az)
    zp = np.ones_like(t_az) * platform_alt

    # Assume target at initial position at center range bin
    # Map range bin to approximate slant-range (simplified)
    zt = 0.0  # target at sea level
    # across-track x0 and along-track y0 from image center
    x0 = 0.0
    y0 = 15000.0  # example, approx center in along-track direction

    xt = x0 + vx * t_az
    yt = y0 + vy * t_az
    zt_arr = np.ones_like(t_az) * zt

    R = np.sqrt((xp - xt)**2 + (yp - yt)**2 + (zp - zt_arr)**2)

    # Compute round-trip phase
    phi = (4.0 * np.pi / lam) * R
    phi_centered = phi - phi[na//2]  # subtract center to avoid shifting target

    # Step 4: construct phase correction
    phase_corr = np.exp(-1j * phi_centered)
    S_corr = S_est * phase_corr[:, None]

    # Step 5: recompress azimuth if we undid FFT
    if undo_fft_azimuth:
        img_refocused = fftshift(ifft(ifftshift(S_corr, axes=0), axis=0), axes=0)
    else:
        img_refocused = S_corr
...
# img = your 64x64 complex Sentinel-1 imagette
vx = 8.0   # across-track (m/s)
vy = 80.0   # along-track (m/s)

img_refocused, info = refocus_with_velocity(imgVV_ship, vx, vy)
