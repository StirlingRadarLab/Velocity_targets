...
#            extent=[0, imgVV.shape[1], freq_axis[0], freq_axis[-1]])
...
plt.subplot(1,2,1)
plt.plot(np.real(imgVV[:, imgVV.shape[1]//2]))
plt.title("Example Azimuth Line (Time Domain)")
plt.xlabel("Azimuth Samples")
plt.ylabel("Amplitude")

# plt.subplot(1,2,2)
# plt.plot(freq_axis, 10*np.log10(np.abs(azimuth_fft[:, imgVV.shape[1]//2])**2 + 1e-12))
...
# PARAMETERS
eps = 1e-12
min_peak_ratio = 5.0  # amplitude ratio threshold for trusting a peak (tweak)
median_kernel = (9, 9)  # neighborhood to compute background (azimuth x range)

N_az, N_rg = imgVV.shape

# Compute azimuth FFT (shifted) for all range columns
az_fft = np.fft.fftshift(np.fft.fft(imgVV, axis=0), axes=0)  # shape (N_az, N_rg)
power2d = np.abs(az_fft)**2

# freq axis
df = prf / N_az
freq_axis = (np.arange(N_az) - N_az//2) * df

# 1) compute doppler centroid per range column (peak freq), and peak power metrics
doppler_per_col = np.zeros(N_rg)
peak_power = np.zeros(N_rg)
mean_power = np.zeros(N_rg)

for rg in range(N_rg):
    ps = power2d[:, rg]
    idx = np.argmax(ps)
    doppler_per_col[rg] = freq_axis[idx]
    peak_power[rg] = ps[idx]
    mean_power[rg] = np.mean(ps)

# 2) quality mask: require peak >> mean (reduces speckle false peaks)
peak_ratio = (peak_power + eps) / (mean_power + eps)
qual_mask = peak_ratio > min_peak_ratio

# 3) compute background Doppler by median filtering across range (and optionally azimuth)
#    Here we smooth across range to capture large-scale centroid features
bg_doppler = median_filter(doppler_per_col, size=median_kernel[1])  # 1D median across range

# 4) residual Doppler (target) and velocity
residual_doppler = doppler_per_col - bg_doppler
v_residual = -(lambda_c / 2.0) * residual_doppler  # m/s

# mask unreliable estimates
v_residual_masked = np.where(qual_mask, v_residual, np.nan)

# ---------- PLOTTING DIAGNOSTICS ----------
plt.figure(figsize=(12,8))

plt.subplot(3,1,1)
plt.plot(freq_axis, 10*np.log10(np.mean(power2d, axis=1) / np.max(np.mean(power2d, axis=1))), label='Averaged spectrum (dB)')
plt.axvline(np.median(doppler_per_col), color='r', linestyle='--', label='Median centroid')
plt.xlabel('Azimuth freq [Hz]')
plt.ylabel('Normalized power [dB]')
plt.title('Averaged Azimuth Power Spectrum')
plt.legend(); plt.grid(True)

plt.subplot(3,1,2)
plt.plot(doppler_per_col, label='Measured centroid per range')
plt.plot(bg_doppler, label='Background (median filtered)', linewidth=2)
plt.ylabel('Doppler [Hz]')
plt.title('Doppler centroid across range')
plt.legend(); plt.grid(True)

plt.subplot(3,1,3)
plt.plot(v_residual, label='Residual velocity (unmasked)')
plt.plot(np.where(qual_mask, v_residual, np.nan), 'r.', label='High-confidence residuals')
plt.xlabel('Range pixel')
plt.ylabel('Velocity [m/s]')
plt.title('Residual velocity after background subtraction')
plt.legend(); plt.grid(True)

plt.tight_layout()
plt.show()

# Additional visualization: 2D map of magnitude and detected residuals
amp = np.abs(VV)
amp_mean_range = np.mean(amp, axis=0)

plt.figure(figsize=(10,4))
plt.subplot(1,2,1)
plt.plot(amp_mean_range / np.max(amp_mean_range))
plt.title('Normalized mean amplitude vs range')
plt.xlabel('Range pixel'); plt.grid(True)

plt.subplot(1,2,2)
plt.plot(v_residual_masked, '.', markersize=3)
plt.title('Masked residual velocity (trusted points)')
plt.xlabel('Range pixel'); plt.ylabel('Velocity [m/s]'); plt.grid(True)
plt.tight_layout()
plt.show()