...
# Helper functions for older NumPy (axis-specific fftshift/ifftshift)
def fftshift_axis0(x):
    n = x.shape[0]
    return np.concatenate((x[n//2:], x[:n//2]), axis=0)

def ifftshift_axis0(x):
    n = x.shape[0]
    return np.concatenate((x[n - n//2:], x[:n - n//2]), axis=0)

def demo_on_synthetic_motion(na=512, nr=64, vx_ship=8.0, vy_ship=0.0):
    """
    Generate a 2D SAR imagette with a moving ship and show defocus and refocusing.
    Compatible with older NumPy versions (no axis argument in fftshift/ifftshift).

    Parameters:
    -----------
    na : int
        Number of azimuth samples (slow-time)
    nr : int
        Number of range samples
    vx_ship : float
        Across-track velocity of ship (m/s)
    vy_ship : float
        Along-track velocity of ship (m/s)
    """

    # Slow-time vector
    t_az = (np.arange(na) - na//2) / PRF_DEFAULT
    ...
    # Target (ship)
    x0, y0, z0 = 0.0, 15000.0, 0.0
    xt = x0 + vx_ship * t_az
    yt = y0 + vy_ship * t_az
    zt = np.ones_like(t_az) * z0

    # Initialize slow-time x range SLC
    S_est = np.zeros((na, nr), dtype=np.complex128)
    nr_idx = nr // 2  # target at center range bin
    for i in range(na):
        R = np.sqrt((xp[i] - xt[i])**2 + (yp[i] - yt[i])**2 + (zp[i] - zt[i])**2)
        S_est[i, nr_idx] = np.exp(-1j * 4.0 * np.pi * R / LAMBDA)

    # ----------------------------------------------------
    # Simple azimuth matched filter (phase-only) to compress
    # ----------------------------------------------------
    R0 = np.sqrt((0 - x0)**2 + (0 - y0)**2 + (h - z0)**2)
    ka = -2 * Vp**2 / (LAMBDA * R0)  # Doppler rate approx
    az_mf = np.exp(1j * np.pi * ka * t_az**2)

    S_corr = S_est * az_mf[:, None]

    # Apply azimuth compression using helper functions
    img_focused = np.abs(fftshift_axis0(np.fft.ifft(ifftshift_axis0(S_corr))))

    # ----------------------------------------------------
    # Refocus using exact velocity (phase correction)
    # ----------------------------------------------------
    R_true = np.sqrt((xp - xt)**2 + (yp - yt)**2 + (zp - zt)**2)
    phi_corr = np.exp(1j * 4 * np.pi / LAMBDA * (R0 - R_true))
    S_refocused = S_corr * phi_corr[:, None]

    img_refocused = np.abs(fftshift_axis0(np.fft.ifft(ifftshift_axis0(S_refocused))))

    # Plot results
    plt.figure(figsize=(10,4))
    plt.subplot(1,2,1)
    plt.imshow(img_focused, cmap='gray')
    plt.title(f"Focused Imagette (vx={vx_ship}, vy={vy_ship})")
    plt.axis('off')

    plt.subplot(1,2,2)
    plt.imshow(img_refocused, cmap='gray')
    plt.title("Refocused Imagette (phase correction applied)")
    plt.axis('off')
    plt.show()

    return img_focused, img_refocused
...
# Simulate a moving ship at 50 m/s along-track
img_def, img_ref = demo_on_synthetic_motion(na=512, nr=64, vx_ship=0.0, vy_ship=50.0)
...
# -------------------------------------------------------
# SAR and simulation parameters (approx Sentinel-1 like)
# -------------------------------------------------------
c = 299792458.0
fc = 5.405e9             # Hz (C-band)
lam = c / fc
B = 20e6                 # Hz (range bandwidth)
Tr = 50e-6               # s (pulse duration)
k = B / Tr               # chirp slope
fs = 40e6                # Hz (range sampling)
PRF = 1700.0             # Hz (azimuth PRF)
Vp = 7090.0              # m/s (platform velocity)
h = 693000.0             # m (platform altitude)

# Grid sizes
na = 512                 # azimuth samples (slow time)
nr = 1024                # range samples (fast time)
...
# -------------------------------------------------------
x0, y0, z0 = 0.0, 15000.0, 0.0     # initial pos (m)
vx_true, vy_true = 8.0, 0.0        # true target velocity (m/s)
RCS = 1.0
...
# True slant range
R = np.sqrt((xp - xt)**2 + (yp - yt)**2 + (zp - zt)**2)
tau0 = 2.0 * R / c  # round-trip delay

# -------------------------------------------------------
# Step 1: Generate synthetic raw echo (baseband)
# -------------------------------------------------------
S = np.zeros((na, nr), dtype=np.complex64)

# Approximate each azimuth line as a delta pulse with phase
for i in range(na):
    idx = int(np.round(tau0[i] * fs))
    if 0 <= idx < nr:
        phase = np.exp(-1j * 4.0 * np.pi * R[i] / lam)
        S[i, idx] += RCS * phase

# -------------------------------------------------------
# Step 2: Range compression
# -------------------------------------------------------
t_ch = np.arange(-nr/2, nr/2) / fs
tx_chirp = np.exp(1j * np.pi * k * t_ch**2)
mf = np.conj(np.fft.ifftshift(tx_chirp))

S_f = np.fft.fft(S, axis=1)
MF_f = np.fft.fft(mf, n=nr)
S_rc = np.fft.ifft(S_f * MF_f[None, :], axis=1)

# Extract a subset of range bins around target
range_line = np.abs(S_rc[na//2, :])
target_bin = np.argmax(range_line)
S_slc = S_rc[:, target_bin]

# -------------------------------------------------------
# Step 3: Azimuth compression (matched filter) — no motion comp
# -------------------------------------------------------
# Compute azimuth frequency axis
fa = np.fft.fftfreq(na, d=1/PRF)
# Approximate azimuth Doppler phase for stationary target
ka = -2 * Vp**2 / (lam * y0)  # Doppler rate (approx small-angle)
az_filter = np.exp(1j * np.pi * ka * t_az**2)

# Azimuth compression
S_az_fft = np.fft.fftshift(np.fft.fft(np.fft.ifftshift(S_slc)))
img_defocused = np.abs(np.fft.fftshift(np.fft.ifft(np.fft.ifftshift(S_az_fft))))

# -------------------------------------------------------
# Step 4: Apply motion compensation / refocusing
# -------------------------------------------------------
# Try estimated velocity (correct value -> focused)
vx_est, vy_est = vx_true, vy_true

xt_est = x0 + vx_est * t_az
yt_est = y0 + vy_est * t_az
zt_est = 0
Rt_est = np.sqrt((xp - xt_est)**2 + (yp - yt_est)**2 + (zp - zt_est)**2)
R_ref = Rt_est[len(t_az)//2]
phi_corr = (4 * np.pi / lam) * (R_ref - Rt_est)
S_corr = S_slc * np.exp(1j * phi_corr)

S_corr_fft = np.fft.fftshift(np.fft.fft(np.fft.ifftshift(S_corr)))
img_refocused = np.abs(np.fft.fftshift(np.fft.ifft(np.fft.ifftshift(S_corr_fft))))
...
plt.subplot(1, 2, 1)
plt.plot(img_defocused / np.max(img_defocused))
plt.title("Azimuth profile (no motion comp)")
plt.xlabel("Azimuth samples")
plt.subplot(1, 2, 2)
plt.plot(img_refocused / np.max(img_refocused))
plt.title("Refocused (vx=%.1f m/s)" % vx_est)
plt.xlabel("Azimuth samples")
plt.tight_layout()
plt.show()
