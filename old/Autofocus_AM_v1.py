...
max_val = 10       # end value

# Linear array (evenly spaced)
linear = np.linspace(min_val, max_val, dim[0])
...
# imgVV_FFT_corr = imgVV_FFT*np.exp(-1j*spectrum_medio*np.pi*10)
trans = "linear"
trans = "quadratic"
if trans == "linear": 
    imgVV_FFT_corr = imgVV_FFT*np.exp(-1j*linear*np.pi*10)
elif trans == "quadratic":
    imgVV_FFT_corr = imgVV_FFT*np.exp(-1j*quadratic*np.pi)
# the linear only makes a shift in the image
# we need to use quadratic
