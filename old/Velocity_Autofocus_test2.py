...
# ----------------------------
def demo_on_synthetic(na=64, nr=64, prf=PRF_DEFAULT):
    """
    Create a simple synthetic SLC for a moving point target and show refocusing.
    This illustrates the algorithm end-to-end using the function above.
    """
    # synthetic slow-time and range axes
    t_az = (np.arange(na) - na//2) / prf
    # Create simple platform & target geometry for phase law:
    Vp = 7090.0; h = 693000.0
    xp = Vp * t_az
    yp = np.zeros_like(t_az)
    zp = np.ones_like(t_az) * h

    # target true motion (set a moderate along-track speed to get defocus)
    x0, y0, z0 = 0.0, 15000.0, 0.0
    vx_true, vy_true = 0.0, 8.0   # m/s

    xt = x0 + vx_true * t_az
    yt = y0 + vy_true * t_az
    zt = np.ones_like(t_az) * z0

    # build synthetic slow-time x range S_est directly (simple delta in range)
    # For simplicity make a single strong range bin at index nr//2
    nr_idx = nr // 2
    S_est = np.zeros((na, nr), dtype=np.complex128)
    for i in range(na):
        R = np.sqrt((xp[i] - xt[i])**2 + (yp[i] - yt[i])**2 + (zp[i] - zt[i])**2)
        phase = np.exp(-1j * 4.0 * np.pi * R / LAMBDA)
        S_est[i, nr_idx] = phase

    # create a "focused image" by applying a naive azimuth FFT compression (like many demos)
    img_focused = fftshift(ifft(ifftshift(S_est, axes=0), axis=0), axes=0)

    # Now run the Doppler refocus on this imagette (simulate user only having focused imagette)
    img_refocused, info = doppler_refocus(img_focused, prf=prf, fc=FC,
                                          poly_order=2, search_range_halfwidth=0,
                                          undo_fft_azimuth=True)
    # display
    plt.figure(figsize=(8,4))
    plt.subplot(1,2,1)
    plt.title("Original (focused imagette magnitude)")
    plt.imshow(np.abs(img_focused), cmap='gray'); plt.axis('off')
    plt.subplot(1,2,2)
    plt.title("After Doppler refocus magnitude")
    plt.imshow(np.abs(img_refocused), cmap='gray'); plt.axis('off')
    plt.show()

    return img_focused, img_refocused, info
